...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $admin_mail_sent = wp_mail($admin_email, $email_subject, $email_message, $admin_headers);
    error_log('Admin email send result: ' . ($admin_mail_sent ? 'Success' : 'Failed'));

    // Send thank you email to user
    $user_subject = "Thank you for contacting SheWings Foundation";
    $user_message = "Dear $first_name,\n\n";
    $user_message .= "Thank you for contacting SheWings Foundation. We have received your message and will get back to you shortly.\n\n";
    $user_message .= "Here's a copy of your message for your reference:\n\n";
    $user_message .= "Subject: $subject\n";
    $user_message .= "Message: $message\n\n";
    $user_message .= "Best regards,\nSheWings Foundation Team";

    // Set up user email headers
    $user_headers = array(
        'Content-Type: text/plain; charset=UTF-8',
        'From: SheWings Foundation <info@shewings.com>',
    ...
    error_log('Attempting to send thank you email to user: ' . $email);
    $user_mail_sent = wp_mail($email, $user_subject, $user_message, $user_headers);
    error_log('User email send result: ' . ($user_mail_sent ? 'Success' : 'Failed'));

    // If either email fails, log the error
